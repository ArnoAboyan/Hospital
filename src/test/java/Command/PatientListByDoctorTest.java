...
class PatientListByDoctorTest {
    ...
    @Test
    void executeTestWithOutSortForAdmin() throws SQLException, DAOException, CommandException {
        ...
        when(req.getParameter("patientsfordoctorid")).thenReturn("8");
        ...
        when(con.prepareStatement(AttributFinal.GET_ALL_PATIENT_LIMIT_BY_ID)).thenReturn(psOne);
        ...
        when(con.prepareStatement(AttributFinal.COUNT_OF_PATIENT_BY_ID)).thenReturn(psTwo);
        when(psTwo.executeQuery()).thenReturn(rsTwo);
        when(rsTwo.next()).thenReturn(true).thenReturn(false);
        when(rsTwo.getInt(1)).thenReturn(1);

        PatientListByDoctor patientListByDoctor = new PatientListByDoctor();


        String actual = patientListByDoctor.execute(req, resp);
        String expected = "patientlistbydoctoradmin.jsp";
        assertEquals(actual, expected);
    }


    @Test
    void executeTestWithOutSortForDoctor() throws SQLException, DAOException, CommandException {
        HttpSession session = mock(HttpSession.class);
        ...
        String actual = patientListByDoctor.execute(req, resp);
        String expected = "patientlistbydoctor.jsp";
        assertEquals(actual, expected);
    }


    @Test
    void executeWithSortTest() throws DAOException, CommandException, SQLException {
        HttpSession session = mock(HttpSession.class);
        when(session.getAttribute("currentUser")).thenReturn(doctorAdmin);
